...
def render_group(group: dict):
    ...
    # FX implication
    # For Gemini-sourced events: raw_detail already contains the fx_implication
    # generated by 2.5 Flash — use it directly and preserve the Groq 70b budget
    # (16K tokens/day) exclusively for NSE RSS / yfinance / FMP cards.
    fx_html    = ""
    raw_detail = lead.get("raw_detail", "")
    if lead.get("_gemini_verified") and raw_detail:
        fx_html = (f'<div class="bb-fx">'
                   f'<span class="bb-fx-lbl">FX ▶</span>{raw_detail}</div>')
    elif lead["is_scb_client"] and score >= 50 and GROQ_API_KEY and client:
